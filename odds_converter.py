...
def convertor():
  while True:
    a = int(input('Odds boxer1 win: '))
    b = int(input('Odds boxer2 win: '))
    c = int(input('Odds draw: '))
    d = input('Odds in american (a), in fractional (f): ')
    lista = [a,b,c]
    if d == 'a':
        d = 'american'
    elif d == 'f':
    ...
    function_odds(lista, d)
    continuar = input('\nContinue? (y or enter = yes, n = no): ')
    if continuar == 'n':
      break
    else:
      print('\n')
